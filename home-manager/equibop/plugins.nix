...

{
  config = {
    ...
    programs.equibop.equicord.settings.plugins = {
      ...
      SilentMessageToggle = {
        enabled = true;
        persistState = "restarts"; # How to persist the silent message toggle state ("none": Don't persist (reset on channel change), "channels": Persist between channels, "restarts": Persist between channels and restarts).
